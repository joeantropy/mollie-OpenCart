...
class PaymentRouteCollectionFactory extends \Mollie\Api\Factories\Factory
{
    public function create() : DataCollection
    {
        $paymentRoutes = \array_map(function ($item) {
            ...
            return new PaymentRoute(\Mollie\Api\Factories\MoneyFactory::new(Arr::get($item, 'amount'))->create(), Arr::get($item, 'destination.organizationId'), Utility::transform(Arr::get($item, 'delayUntil'), fn($value) => new Date($value), Date::class));
        }, $this->get());
