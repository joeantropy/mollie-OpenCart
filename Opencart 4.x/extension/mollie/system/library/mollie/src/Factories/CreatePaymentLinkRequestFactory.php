...
class CreatePaymentLinkRequestFactory extends \Mollie\Api\Factories\RequestFactory
{
    public function create() : CreatePaymentLinkRequest
    {
        return new CreatePaymentLinkRequest($this->payload('description'), $this->transformFromPayload('amount', fn($amount) => \Mollie\Api\Factories\MoneyFactory::new($amount)->create()), $this->payload('redirectUrl'), $this->payload('webhookUrl'), $this->payload('profileId'), $this->payload('reusable'), $this->transformFromPayload('expiresAt', fn($date) => $this->getExpiresAt($date), DateTime::class), $this->payload('allowedMethods'), $this->payload('sequenceType'), $this->payload('customerId'), $this->transformFromPayload('lines', fn($items) => !empty($items) ? \Mollie\Api\Factories\OrderLineCollectionFactory::new($items)->create() : null), $this->transformFromPayload('billingAddress', fn($item) => Address::fromArray($item)), $this->transformFromPayload('shippingAddress', fn($item) => Address::fromArray($item)), $this->transformFromPayload('minimumAmount', fn($amount) => \Mollie\Api\Factories\MoneyFactory::new($amount)->create()));
    }
    /**
