...
final class CurlMollieHttpAdapter implements HttpAdapterContract
{
    ...
    protected function send(PendingRequest $pendingRequest) : array
    {
        ...
        try {
            $curl = \Mollie\Api\Http\Adapter\CurlFactory::new($request->getUri(), $pendingRequest)->withHeaders($pendingRequest->headers()->all())->withMethod($pendingRequest->method(), $request->getBody())->create();
            $response = \curl_exec($curl);
            if ($response === \false) {
                $curlErrorNumber = \curl_errno($curl);
                $curlErrorMessage = 'Curl error: ' . \curl_error($curl);
                throw \Mollie\Api\Http\Adapter\CurlErrorMapper::toException($curlErrorNumber, $curlErrorMessage, $pendingRequest);
            }
