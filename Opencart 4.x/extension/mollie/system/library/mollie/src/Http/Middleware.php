...
class Middleware
{
    ...
    {
        $this->onRequest->add(static function (\Mollie\Api\Http\PendingRequest $pendingRequest) use($callback) : \Mollie\Api\Http\PendingRequest {
            $result = $callback($pendingRequest);
            if ($result instanceof \Mollie\Api\Http\PendingRequest) {
                return $result;
            }
    ...
    public function onResponse(callable $callback, ?string $name = null, string $priority = MiddlewarePriority::MEDIUM) : self
    {
        ...
        $this->onResponse->add(static function ($response) use($callback) {
            ...
            return $result instanceof \Mollie\Api\Http\Response || $result instanceof ViableResponse ? $result : $response;
        }, $name, $priority);
        return $this;
    }
    ...
    public function executeOnRequest(\Mollie\Api\Http\PendingRequest $pendingRequest) : \Mollie\Api\Http\PendingRequest
    {
     ...
     */
    public function executeOnResponse(\Mollie\Api\Http\Response $response)
    {
