...
class MollieHttpAdapterPicker implements MollieHttpAdapterPickerContract
{
    ...
    public function pickHttpAdapter($httpClient) : HttpAdapterContract
    {
        ...
        if ($httpClient instanceof \MollieVendor\GuzzleHttp\ClientInterface) {
            return new \Mollie\Api\Http\Adapter\GuzzleMollieHttpAdapter($httpClient);
        }
    ...
    private function createDefaultAdapter() : HttpAdapterContract
    {
        if ($this->guzzleIsDetected()) {
            return \Mollie\Api\Http\Adapter\GuzzleMollieHttpAdapter::createClient();
        }
        return new \Mollie\Api\Http\Adapter\CurlMollieHttpAdapter();
    }
    private function guzzleIsDetected() : bool
    {
        return \interface_exists('\\' . \MollieVendor\GuzzleHttp\ClientInterface::class);
    }
}
