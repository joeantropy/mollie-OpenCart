...
class CurlFactory
{
    ...
    public static function new(string $url, PendingRequest $pendingRequest) : self
    {
        ...
        if ($handle === \false) {
            throw new \Mollie\Api\Http\Adapter\CurlInitializationException($pendingRequest, 'Failed to initialize CURL');
        }
    ...
    private function setOption(int $option, $value) : void
    {
        if (\curl_setopt($this->handle, $option, $value) === \false) {
            throw new \Mollie\Api\Http\Adapter\CurlInitializationException($this->pendingRequest, \sprintf('Failed to set CURL option %d', $option));
        }
    }
