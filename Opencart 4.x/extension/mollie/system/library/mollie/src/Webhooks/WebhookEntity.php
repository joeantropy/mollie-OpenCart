...
class WebhookEntity
{
     ...
     */
    public static function create($entityData) : \Mollie\Api\Webhooks\WebhookEntity
    {
        $data = Utility::transform($entityData, fn($data) => \is_object($data) ? (array) $data : $data, null, []);
        $resourceType = Arr::get($data, 'resource');
        $id = Arr::get($data, 'id');
        return new \Mollie\Api\Webhooks\WebhookEntity($resourceType, $id, $data);
    }
    ...
    {
        return $connector->send((new DynamicGetRequest($href))->setHydratableResource($targetClass));
    }
}
