...
class ResourceResponseBuilder
{
     ...
     */
    public function create() : \Mollie\Api\Fake\MockResponse
    {
        $baseName = Str::snake(Utility::classBasename($this->resourceClass), '-');
        $sampleContents = \json_decode(\Mollie\Api\Fake\FakeResponseLoader::load($baseName), \true);
        ...
        return new \Mollie\Api\Fake\MockResponse($data);
    }
    public function __call($method, $parameters)
    {
        if ($method === 'embed') {
            ...
            if (!isset($this->embeddedBuilders[$collectionClass])) {
                $this->embeddedBuilders[$collectionClass] = new \Mollie\Api\Fake\ListResponseBuilder($collectionClass);
            }
